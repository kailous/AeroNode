...
static const unsigned char image_Rpc_active_bits[] PROGMEM = {
    0x07, 0x35, 0x47, 0x40, 0x01, 0x71, 0x56, 0x70};
static const unsigned char image_BLE_beacon_bits[] PROGMEM = {
    0x22, 0x49, 0x55, 0x49, 0x2a, 0x08, 0x08, 0x3e};
static const unsigned char image_GameMode_bits[] PROGMEM = {
    0x20, 0x00, 0xfe, 0x03, 0xfb, 0x07, 0x71, 0x05,
    0xfb, 0x07, 0x8f, 0x07, 0x07, 0x07, 0x03, 0x06};
static const unsigned char image_Alert_bits[] PROGMEM = {
...
OledDisplay::OledDisplay(uint8_t sdaPin, uint8_t sclPin)
    : u8g2(U8G2_R2, /* clock=*/sclPin, /* data=*/sdaPin,
           /* reset=*/U8X8_PIN_NONE) {}
...
void OledDisplay::showWifiTimeoutError() { showError("WiFi Timeout"); }

void OledDisplay::showMpuConnectionError() { showError("MPU Error", "E-01"); }
...
void OledDisplay::showServerReady(const String &ssid, const IPAddress &ip,
                                  uint16_t port, const String &mac) {
  if (!_available)
    return;
  u8g2.clearBuffer();
  drawHeader();
...
void OledDisplay::showClientConnected(const IPAddress &remoteIp) {
  ...
  char buf[24];
  snprintf(buf, sizeof(buf), "%d.%d.%d.%d", remoteIp[0], remoteIp[1],
...
void OledDisplay::showBleReady(const String &name, const String &mac) {
  if (!_available)
    return;
  ...
  u8g2.setDrawColor(1);
  u8g2.drawStr(5, 46, "NAME");
  drawRightAlignedStr(46, name.c_str());

  // 只显示纯净 MAC 地址
  drawRightAlignedStr(59, mac.c_str());

  u8g2.drawStr(5, 28, "BLE");
  drawRightAlignedStr(28, "ON");

  drawSeparator(32);

  // Draw the Bluetooth beacon icon
  u8g2.setDrawColor(2);
  u8g2.drawXBMP(116, 4, 7, 8, image_BLE_beacon_bits);
