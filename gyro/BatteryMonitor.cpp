...
uint8_t BatteryMonitor::voltageToPercent(float vinVoltage) const {
    if (_fullVoltage <= _emptyVoltage) return 0;

    float pct = (vinVoltage - _emptyVoltage) * 100.0f / (_fullVoltage - _emptyVoltage);
...
BatteryReading BatteryMonitor::read() {
    ...
    reading.a0Voltage = (reading.raw / 1023.0f) * _adcRefVoltage;
